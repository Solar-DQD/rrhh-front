...
export function ActiveFilters({
    ...
    filters: { key: string, variant: 'text' | 'select' | 'toggle', util?: (id: number) => string, name?: string }[],
    activeFilters: { [key: string]: string | number | boolean },
        ...
        <div className='shrink-0 w-full'>
            <div className='flex flex-row flex-wrap gap-2'>
                {Object.entries(activeFilters).map(([key, value]) => (
                    value !== 0 && value !== '' && value !== null && value !== false && (
                        <div key={key} className='flex items-center gap-2 border-2 border-gray-700 pl-2 pr-1 py-1 rounded'>
                            <span className='text-gray-800 text-xs sm:text-sm'>
                                {(() => {
                                    ...
                                    if (filter.variant === 'toggle') return `${value ? filter.name : ''}`
                                })()}
                            </span>
                            <IconButton
